...
from model import build_inception_time
...
def train_model(
    ...
    """
    Trains the InceptionTime model on the given data.
    ...
    weights_inception = WEIGHTS_PATH + 'InceptionTime.h5'
    ...
    with open(TRAIN_HISTORY + "HistoryInceptionDict", "wb") as file_pi:
...
def define_model(train_data_hc: tf.data.Dataset) -> keras.Model:
    """
    Defines and loads a pre-trained InceptionTime model.
